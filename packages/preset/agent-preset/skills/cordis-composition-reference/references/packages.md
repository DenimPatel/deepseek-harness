...
| `@deepseek-ai/dsh-api-git-worktree-controller` | no | Git worktree Remote commands: create, status, merge, and discard over Typert Remote |
...
| `@deepseek-ai/dsh-client-ui-observability` | no | Read-side session observability dashboard: token and time breakdowns, throughput trend, provider/model metrics, and the sub-agent tree |
...
| `@deepseek-ai/dsh-client-ui-step-mode` | no | Web step-mode composer takeover, composer Step button, and Observability pause controls |
...
| `@deepseek-ai/dsh-client-ui-workspace-worktree` | no | Parallel edits per workspace: worktree rows, the New-session-in-worktree entry, and the Session-header Merge/Discard control |
...
| `@deepseek-ai/dsh-step-mode` | yes | Step execution mode: hold each model request and tool dispatch for human step-through |
...
| `@deepseek-ai/dsh-session-stats` | yes | Whole-log conversation counts and wall times projection (sessionStats) for the DeepSeek Harness |
...
| `@deepseek-ai/dsh-workspace-worktree` | yes | Git worktree service (ctx.gitWorktree): create, inspect, merge, and remove linked checkouts beside a repository through shell-free argv subprocesses |
