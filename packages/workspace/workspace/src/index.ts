...
import type {
  SessionActivity, Workspace, WorkspaceId as WorkspaceIdBrand, WorkspaceWorktree, WorkspaceWorktreeCreate,
} from './types.ts'

export type {
  SessionActivity, SessionActivityItem, SessionActivityKind, SessionActivityKindMap, Workspace,
  WorkspaceWorktree, WorkspaceWorktreeCreate,
} from './types.ts'
...
export class WorkspaceRegistry extends Service {
  ...
  initializeDefault(resolveDirectory: () => Promise<{ path: string; title: string }>): Promise<Workspace | undefined> {
    return this.enqueueOperation(async () => {
      ...
      return this.createCanonical(canonical, title, { firstUse: true })
    })
  }
  ...
  private async createCanonical(
    canonical: string,
    title?: string,
    options: { readonly firstUse?: boolean; readonly worktree?: WorkspaceWorktree } = {},
  ): Promise<WorkspaceEntity> {
    const { firstUse = false, worktree } = options
    for (const entity of this.entities.values()) {
