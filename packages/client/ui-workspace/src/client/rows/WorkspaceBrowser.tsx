...
> & PropsRenderSlots<
  ...
  | 'sidebar.workspaces.projectActions' | 'sidebar.workspaces.worktreeRow'
> & {
  ...
  /** Nest Workspaces under their nearest registered ancestors; worktrees nest under their parents regardless. */
...
function SessionTree({
  ...
  const parents = useMemo(() => {
    const registered = new Set<string>(workspaces.map(workspace => workspace.workspaceId))
    ...
    return new Map<string, WorkspaceId | undefined>(workspaces.map((workspace) => {
      // A worktree always nests under its registered parent; the worktree
      // service refuses a linked worktree as another's repository, so the
      // chain has no cycle. A worktree whose parent is absent stays top level.
      const worktreeParent = workspace.worktree?.parentWorkspaceId
      if (worktreeParent !== undefined && worktreeParent !== workspace.workspaceId && registered.has(worktreeParent)) {
        return [workspace.workspaceId, worktreeParent]
      }
      if (!nestWorkspaces) return [workspace.workspaceId, undefined]
