...
import type {
  MenuOpenState, WorkspaceBrowserProps, WorkspaceProjectActionsOwnerProps, WorkspaceWorktreeRowOwnerProps,
...
export function ProjectRowItem({
  group, containsCurrentDescendant = false, onToggle, onCreate, actions, renderActions, drag, home, t,
}: {
  group: GroupNode
...
/**
 * One worktree row, nested under its parent project by the group tree: the group expander, the
...
export function WorktreeRowItem({ group, onToggle, renderRow, t }: {
  ...
  return (
    <div
      className={clsx(css.projectRow, group.containsCurrent && css.worktreeRowCurrent)}
      data-row-key={`workspace:${group.key}`}
      role="treeitem"
