...
describe('WorkspaceBrowser', () => {
  ...
  it('nests a worktree under its registered parent in every grouping and keeps an orphan at top level', () => {
    const worktreeOf = (id: string, parentId: string, branch: string): WorkspaceView => ({
      ...workspace(id, [`${id}-session`], id),
      path: `/projects/repo.worktrees/${id}`,
      worktree: {
        parentWorkspaceId: wid(parentId), repoPath: '/projects/repo', branch, baseBranch: 'main', baseRevision: 'a1b2c3d4',
      },
    })
    const workspaces = hook(workspaceState([
      { ...workspace('repo', [], 'Repo'), path: '/projects/repo' },
      worktreeOf('fix', 'repo', 'dsh/fix'),
      worktreeOf('orphan', 'gone', 'dsh/orphan'),
    ]))
    const sessions = hook(sessionState([summary('fix-session', 1), summary('orphan-session', 2)]))
    mount({ useWorkspaces: workspaces, useSessions: sessions })
    const parentSection = () => screen.getByText('Repo').closest<HTMLElement>('[class*="groupSection"]')!
    // A parent with a worktree child opens by default, like a tree ancestor.
    expect(within(parentSection()).getByText('dsh/fix')).toBeTruthy()
    expect(within(parentSection()).queryByText('dsh/orphan')).toBeNull()
    expect(screen.getByText('dsh/orphan')).toBeTruthy()
    fireEvent.click(screen.getByText('Repo'))
    expect(screen.queryByText('dsh/fix')).toBeNull()
  })
