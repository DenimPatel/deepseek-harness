...
describe('worktree grouping', () => {
  it('carries each worktree descriptor on its own flat group', () => {
    ...
    expect(groups.map(group => group.key)).toEqual(['project', 'wt-one'])
    expect(groups[0]!.worktree).toBeUndefined()
    expect(groups[1]!.worktree?.parentWorkspaceId).toBe(wid('project'))
    expect(groups[1]!.worktree?.branch).toBe('dsh/one')
    expect(groups[1]!.sessions.map(session => session.id)).toEqual([sid('in-one')])
  })
  ...
  it('labels a worktree session with its parent title and branch in search', () => {
    ...
    const inside = { ...summary('inside', 1), displayTitle: 'Find me' }
    const sessions = list(inside)

    const labelled = deriveSearchResults(
      sessions, [parent, child], 'fix-login', noArchive, 'default', noAttention, { items: [], hasMore: false }, 10,
    )
    expect(labelled.items.map(item => item.workspace)).toEqual(['Alpha · dsh/fix-login'])

    const parentless = deriveSearchResults(
      sessions, [child], 'Find me', noArchive, 'default', noAttention, { items: [], hasMore: false }, 10,
    )
