...
流程视图读取轨迹席位，渲染 harness 自身的轮次：已加载的系统提示、上下文注入、用户与插话消息、助手正文及其思考、工具调用与结果、命令运行、压缩标记、模型重试与轮次失败——每行都带步骤、工具目录大小、提示变更与状态，并可展开到精确记录文本。请求行可展开为该请求记录的配置、所提供工具目录、系统提示，以及——当结算报告了用量时——其令牌分桶与缓存命中率；工具行可展开为原始参数、被预览截断的输出以及任何嵌套派发调用。工具行自身不声明轮次——工具生命周期与步骤边界分开组装——因此它取发出它的那条记录所属的轮次与步骤。控件会持续向上翻页，直到窗口抵达日志起点。
